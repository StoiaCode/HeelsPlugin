  ...
  public class PluginUI
  {
    ...
    public void DrawConfiguration()
    {
      ...
      if (ImGui.Begin("HeelsPlugin Config", ref visible))
      {
        ...
        if (Plugin.Configuration.disableSit) {
		  ImGui.SameLine();
		  ...
		  ImGui.InputFloat($"##Height {Plugin.Configuration.customSit}", ref Plugin.Configuration.customSit, 0.01f, 0.05f, "%.3f");
          if (ImGui.IsItemHovered()) ImGui.SetTooltip("Currently in Beta, will not Sync with Mare.");
        } else {
          Plugin.Configuration.customSitEnable = false;
        }
